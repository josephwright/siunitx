...
controlled by the \opt{prefix-mode} option, which takes the values \opt{symbol}
and \opt{power}. The treatment of kilograms can be set using
\opt{extract-mass-in-kilograms}: when \opt{true}, the \emph{kilo} prefix is
retained as part of the unit. This will mean that all grams are converted to
kilograms.
\begin{LaTeXdemo}
  \qty{10}{\kilo\gram\squared\deci\second} \\
  \qty[prefix-mode = exponent]{10}{\kilo\gram\squared\deci\second}\\
  \qty[prefix-mode = exponent]{7.5}{\gram} \\
