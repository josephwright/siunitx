...
version~$3$ of the package is given.

\subsection{Version~\texorpdfstring{$3.1$}{3.1}}

\subsection{Version~\texorpdfstring{$3.2$}{3.2}}

\begin{itemize}
  \item \enquote{Threshold} approach to producing exponent notation:
    similar to a calculator
  \item Finer control of priting plus signs using new options to split
    mantissa and exponent
  \item Support for updated prefixes introduced by \acro{bipm}
  \item Revised alignment of uncertainties in tabulars
\end{itemize}

\subsection{Version~\texorpdfstring{$3.3$}{3.3}}
...
\begin{itemize}
  \item Support for languages which need a begin as well as end phrase in
    ...
    supporting options
  \item Support for asymmetric uncertainties (tolerances) was added
  \item Rounding was extended to allow truncation (rounding down) for
    both main values and when using the uncertainty part
  \item Finer control of the appearance of brackets in ambiguous
     output was introduced by adding options for different contexts
  \item Control of list and product input and output was extended
  \item Finer control of table alignment setup for bold values was
    introduced
  \item New unit abbreviations for magnetic fields were added
\end{itemize}

\subsection{Version~\texorpdfstring{$3.4$}{3.4}}
