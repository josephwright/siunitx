...
\NewDocumentCommand\version{m}{#1}
...
\usepackage{fancyvrb}
\ExplSyntaxOn
\box_new:N \l__LaTeXdemo_tmp_box
\NewDocumentEnvironment { LaTeXdemo } { O { code~and~example } c }
  {
    \par
    \use:c { __LaTeXdemo_ #1 :n } {#2}
    \par
  }
  { }
\cs_new_protected:cpn { __LaTeXdemo_code~and~example:n } #1
  {
    \hbox_set:Nn \l__LaTeXdemo_tmp_box
      { \vbox:n { \__LaTeXdemo_auxii:n {#1} } }
    \begin { center }
      \dim_compare:nNnTF
        { \box_wd:N \l__LaTeXdemo_tmp_box } > { 0.48 \linewidth }
        {
          \par
          \begin { minipage } { \linewidth }
            \box_use:N \l__LaTeXdemo_tmp_box
          \end { minipage }
          \par
          \begin { minipage } { \linewidth }
            \__LaTeXdemo_auxi:n {#1}
          \end { minipage }
        }
        {
...
\cs_new_protected:cpn { __LaTeXdemo_code~and~float:n } #1
  {
    \__LaTeXdemo_auxi:n {#1}
    \begin { minipage } { \linewidth }
      \__LaTeXdemo_auxii:n {#1}
    \end { minipage }
  }
\cs_new_protected:cpn { __LaTeXdemo_code~only:n } #1
  {
    \begin { minipage } { \linewidth }
      \__LaTeXdemo_auxii:n {#1}
    \end { minipage }
  }
\cs_new_protected:Npn \__LaTeXdemo_auxi:n #1
  {
    \group_begin:
...
\cs_new_protected:Npn \__LaTeXdemo_auxii:n #1
  {
    \__LaTeXdemo_auxi:n
      {
        \tl_to_str:n { \begin { Verbatim } } [ gobble = 2 ] \obeyedline
          #1 \obeyedline
        \token_to_str:N \end \c_left_brace_str Verbatim \c_right_brace_str
          \obeyedline
      }
  }
\ExplSyntaxOff
...
\begin{itemize}
  ...
  \item \cs{duration}\oarg{options}\marg{duration}
...
\begin{function}{\duration}
  \begin{syntax}
    \cs{duration}\oarg{options}\marg{duration}
  \end{syntax}
\end{function}
Durations (of hours, minutes and seconds) can be typeset using the
\cs{duration} command. The \meta{duration} can be given either as a decimal
number or as a semi-colon separated list of hours, minutes and seconds, which
is called \enquote{component format} in this document. The numbers which make
up a duration are processed using the same system as other numbers. However, no
unit prefixes are applied here: as such, conversion from exponent form to unit
prefixes does not apply.
\begin{LaTeXdemo}
  \duration{10}    \\
  \duration{12.3}  \\
  \duration{4,5}   \\
...
the mode. As well as the overall setting, it is possible to apply mode to
numbers and units separately using the \opt{number-mode} and \opt{unit-mode}
options. When both \opt{number-mode} and \opt{unit-mode} are set to \opt{math},
quantities are printed entirely in a single math mode block: this is important
the quantity product and also for tagging.
...
\begin{LaTeXdemo}
  ...
  Still red here!
\end{LaTeXdemo}
Note that the color does \emph{not} apply to text inserted by the package
between numbers, units or quantities. For example, in
\begin{LaTeXdemo}
  \numproduct[color = blue, product-mode = phrase]{1x2}
\end{LaTeXdemo}
only the numerals are colored; coloring all output should be done using the
...
\begin{table}
  ...
  \begin{tabular}{@{}>{\ttfamily}ll>{\ttfamily}l@{}}
    ...
    \midrule
      ...
      group-allow-singleton            & Switch  & true              \\
      ...
      output-decimal-marker            & Literal & \meta{see below}  \\
...
\DescribeOption{final-digit-group-min-size}
Grouping can result in a \enquote{dangling} digit, for example if there are
seven digits in the decimal part with the standard group-of-three approach. To
allow these to be avoided automatically, the setting
\opt{final-digit-group-min-size} is available. Setting this to for example
\opt{2} will mean that the final group is enlarged to respect the minimum size
where this is required to avoid a group size of~1.
\begin{LaTeXdemo}
  \num{0.1234567} \\
  \num[final-digit-group-min-size = 2]{0.1234567}
\end{LaTeXdemo}

\DescribeOption{output-decimal-marker}
The decimal marker used in output is set using the \opt{output-decimal-marker}
option; this can differ from the input marker. The standard value will be either
|.| or |,|: this is set based on the primary language of the document.
\begin{LaTeXdemo}
  \num{1.23} \\
...
This option does \emph{not} apply to complex numbers.
\begin{LaTeXdemo}
  \num{-15673} \\
...
\opt{bracket-negative-numbers} option. This option does \emph{not} apply to
complex numbers.
\begin{LaTeXdemo}
  \num{-15673} \\
...
Duration processing provided by the \cs{duration} function has a set of options
which apply in addition to the general ones set up for number processing.
\begin{table}
  \caption{Angle options.%
    \label{tab:opt:duration}}
  \begin{tabular}{@{}>{\ttfamily}ll>{\ttfamily}l@{}}
    \toprule
      \multicolumn{1}{@{}l}{Option name} &
      Type &
...
option. With the standard setting (\opt{input}), the duration is printed as-given.
...
\begin{LaTeXdemo}
  \duration{8.56}    \\
  \duration{8;33;36} \\
  \duration[duration-mode = component]{8.56}    \\
  \duration[duration-mode = component]{8;33;36} \\
...
\begin{LaTeXdemo}
  ...
  \duration{;;3} \\
  {
    \sisetup{fill-duration-hours}
    \duration{1;;} \\
    \duration{;2;} \\
    \duration{;;3} \\
  }
  {
    \sisetup{fill-duration-minutes}
    \duration{1;;} \\
    \duration{;2;} \\
    \duration{;;3} \\
  }
  {
    \sisetup{fill-duration-seconds}
    \duration{1;;} \\
    \duration{;2;} \\
    \duration{;;3}
  }
...
\DescribeOption{disable-deprecated-commands}
\DescribeOption{disable-deprecated-options}
To allow users to require only interfaces from the current release, the options
\opt{disable-deprecated-options} and \opt{disable-deprecated-commands} are
available. Settings these to \opt{false} will disable options and document
commands that have been removed from version~\version{3}, respectively.
...
\subsection{Version~\version{3.5}}

\begin{itemize}
  \item New \cs{duration} command and supporting options
  \item Support for extending the last group of numbers to avoid a dangling
    digit
  \item Options to suppress interfaces from version~\version{2}
\end{itemize}

\subsection{Version~\version{3.6}}

\begin{itemize}
  \item Print the entire quantity in math mode when both the number and unit
    modes are set to \opt{math} (avoids switching to text mode for the
    quantity product)
  \item Automatic setting of \opt{output-decimal-marker} based on document-wide
    localization information (from \cs{DocumentMetadata}, \pkg{babel} or
    \pkg{polyglossia})
  \item Option \opt{locale} deprecated
  \item MathML intent is marked up for units when used in math mode (with
    Lua\TeX{})
\end{itemize}
...
\end{itemize}
These were removed from the 9th Edition, but were re-added in the June 2026
update in a non-exhaustive table which suggests they may not remain
permanently. As such, the unit macros remain available but issue an information
message on first use. It is recommended that users add their own definitions to
the start of their sources for long-term stability.
...
The package will automatically check the current locale information set in
\cs{DocumentMetadata}, or by \pkg{babel} or \pkg{polyglossia}. It will then use
this to set the appropriate default for the output decimal marker to a point or
a comma.\footnote{This uses the data files from \pkg{babel}, even if it is not
in use in the document.} A user setting of \opt{output-decimal-marker} will be
respected.

The \pkg{translations} package provides a structured framework for
localization of words and phrases. In particular, it offers the
...
result will be localized text. The preamble for this manual loads English,
...
\subsection{Associating regional settings with a \pkg{babel} language%
  \label{sec:hint:babel}}

It is possible to instruct the \pkg{babel} package to switch to a particular
regional setting when changing language. This can be done using the \pkg{babel}
\cs{extras\meta{language}} system. For example, to associate the \texttt{german}
\pkg{babel} language with a comma for decimal marker, the appropriate code would
be
\begin{LaTeXdemo}[code only]
  \addto\extrasgerman{\sisetup{output-decimal-marker = {,}}}
\end{LaTeXdemo}
...
\subsection{Tagging for reuse and accessibly%
  \label{sec:hint:tagging}}

Tagged~\acro{PDF} files allow reuse of the content, particularly important for
accessibility. The \pkg{siunitx} package aims to create output that as far as
possible can be tagged automatically by \LaTeX{} kernel methods by the addition
of
\begin{LaTeXdemo}[code only]
  \DocumentMetadata{tagging = on}
\end{LaTeXdemo}
at the start of sources.

When used with Lua\TeX{}, \pkg{siunitx} will automatically add MathML intent
information to the unit output, meaning that this can be passed through. The
best results will be obtained when both the number and unit in math mode:
