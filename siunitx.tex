...
Numbers are processed with understanding of exponents.
\begin{LaTeXdemo}
  \num{12345,67890} \\
  \num{.3e45}       \\
\end{LaTeXdemo}
...
\enquote{accepted} units~\cite{SI:T6}. Some units are fundamental physical
quantities, and these are non-SI but can be used within the \acro{SI}
(Table~\ref{tab:unit:physical},~\cite{SI:T7}). There are also a set of non-\acro{SI}
...
\acro{CGS} units are sanctioned (Table~\ref{tab:unit:CGS})~\cite{SI:T9}. The
command \cs{percent} is also provided for use in units: this is accepted with
the \acro{SI} as detailed in Section~5.3.7 of the Brochure~\cite{SI:5.3.7}.
\begin{table}
...
\end{table}
\begin{table}
  \caption{Non-\acro{SI} units whose values in \acro{SI} units must be obtained
    experimentally.%
...
\end{table}
\begin{table}
  \caption{Other non-\acro{SI} units.%
...
\end{table}
\begin{table}
  \caption{Non-\acro{SI} units associated with the \acro{CGS}.%
    \label{tab:unit:CGS}}
  \begin{tabular}{@{}lll@{}}
    \toprule
      Unit & Command & Symbol \\
...
for all of the \acro{SI} prefixes (Table~\ref{tab:unit:prefix})~\cite{SI:3.1}.
...
\begin{table}
  ...
  \begin{tabular}{@{}>{\ttfamily}ll>{\ttfamily}l@{}}
    ...
    \midrule
      ...
      print-zero-exponenet     & Switch  & false             \\
...
\begin{table}
  \caption{Output options for lists, products and ranges of numbers and quantities.%
...
\section{Upgrading from version~$2$}
...
definition of the \cs{micro} prefix. As such, the \cs{SIUnitSymbol\dots}
commands and associated options have been removed.
...
binary prefixes and units (Section~\ref{tab:unit:binary}) are the most obvious
