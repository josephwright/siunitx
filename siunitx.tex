...
When the uncertainty is given in brackets, the exact appearance can be set
using the \opt{uncertainty-mode} option, which takes values \opt{compact},
\opt{full} and \opt{mixed}. The standard setting of \opt{compact} prints digits
of uncertainty in the least-significant digits. It does \emph{not} print a
decimal marker if the uncertainty crosses the decimal. The setting \opt{full}
prints the full value of the uncertainty. Finally, \opt{mixed} is available to
print in the \opt{compact} style except where the uncertainty crosses the
decimal, in which case the \opt{full} style is used.
\begin{LaTeXdemo}
  \num{123.45(120)} \\
  \num{0.035(14)}   \\
  ...
  \num{123.45(120)} \\
  \num{0.035(14)}   \\
  \sisetup{uncertainty-mode = mixed}
  \num{123.45(120)} \\
  \num{0.035(14)}   \\
\end{LaTeXdemo}
...
using \opt{angle-symbol-degree}, \opt{angle-symbol-minute} and \opt{angle-symbol-second}.
This is most likely to be used when the definition of the unit macros is altered,
for example to set \cs{arcsecond} as \texttt{as}.
\begin{LaTeXdemo}
  \ang{6;7;6.5} \\
  \sisetup{
...
source. See Section~\ref{sec:print} for more details on the options available
in this area.
